...
export function CategoryPills({categories,selectedCategory,onSelect}){
    ...
    return (
    <>
    <div className="overflow-x-hidden relative [&::-webkit-scrollbar]:hidden" ref={containerRef}>
        ...
        {categories.map(category => (
            <Button variant={selectedCategory === category ? "dark" : "default"} className="py-1 px-3 rounded-lg whitespace-nowrap" onClick={()=>onSelect(category)}>
                {category}
            </Button>
        ))} 
        ...
        {isLeftVisible && <div className="absolute left-0 top-1/2 -translate-y-1/2 bg-gradient-to-r from-white from-50% to-transparent w-24 h-full">
            ...
            onClick={()=>{
                setTranslate(translate => {
                    ...
                    console.log(translate,TRANSLATE_AMOUNT)
                    if(newTranslate <= 0){
                        return 0
                    }
                    return newTranslate
